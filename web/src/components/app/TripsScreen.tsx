...
import { loadTrips, type SavedTrip } from '@/lib/trips-client'
...
export default function TripsScreen({ onOpenJourney, isMobile }: TripsScreenProps) {
  // Locked-in plans (Supabase or local); the prototype's demo trip until one exists.
  const [saved, setSaved] = useState<SavedTrip[] | null>(null)
  useEffect(() => {
    void loadTrips().then(setSaved)
  }, [])
  ...
  const upcoming =
    saved && saved.length > 0
      ? saved.map((t) => ({
          ...
          route: t.route,
          from: t.from.slice(0, 3).toUpperCase(),
          to: t.to.slice(0, 3).toUpperCase(),
          date: t.date,
          depart: t.depart || '—',
          leaveHome: t.leaveHome || '—',
          status: t.status,
        }))
      : demoUpcoming
