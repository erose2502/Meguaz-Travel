...
interface SearchResultsProps {
  solved: SolveResponse | null
  onSelect: (option: PlanOption | null) => void
  onBack: () => void
  isMobile: boolean
}

export default function SearchResults({ solved, onSelect, onBack }: SearchResultsProps) {
  const [priority, setPriority] = useState<Priority>('balanced')
  const listRef = useRef<HTMLDivElement>(null)

  // Live solved options when a brief was captured; the prototype's demo data otherwise.
  const budgetCap = solved?.brief.budget ?? BUDGET_CAP
  const displayOptions: (Option | PlanOption)[] = solved?.options ?? options
  const routeLabel = solved?.routeLabel ?? 'San Francisco → London'
  const arriveByLabel = solved
    ? new Date(solved.brief.arriveBy + 'T00:00:00').toLocaleDateString('en-US', {
        month: 'short',
        day: 'numeric',
      })
    : 'Aug 24 · 9:00 AM'

  const recommendedId = displayOptions.find((o) => o.fits === priority)?.id ?? 'balanced'
    ...
    <div className="min-h-dvh" style={{ background: '#F6F3EA' }}>
        ...
        <div className="mx-auto w-full max-w-3xl">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <h1 className="font-display text-[#0D1B2A] text-xl leading-tight">Making {routeLabel} work</h1>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="mx-auto w-full max-w-3xl px-5 md:px-8 pb-28">
        ...
        <div className="glass-card rounded-3xl p-5 mt-2">
          ...
          <div className="flex items-center gap-3 mb-5">
            <div className="flex-1 rounded-2xl px-4 py-3" style={{ background: 'rgba(20,108,126,0.08)' }}>
              ...
              <p className="text-[#0D1B2A] font-semibold text-sm">{solved?.brief.from ?? 'San Francisco, CA'}</p>
            </div>
            ...
            <div className="flex-1 rounded-2xl px-4 py-3" style={{ background: 'rgba(20,108,126,0.08)' }}>
              ...
              <p className="text-[#0D1B2A] font-semibold text-sm">{solved?.brief.to ?? 'London, UK'}</p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-3 mb-5">
            <div className="rounded-2xl px-4 py-3 flex items-center gap-3" style={{ border: '1px solid rgba(13,27,42,0.1)' }}>
              ...
              <div>
                ...
                <p className="text-[#0D1B2A] font-semibold text-sm">{arriveByLabel}</p>
              </div>
            </div>
            <div className="rounded-2xl px-4 py-3 flex items-center gap-3" style={{ border: '1px solid rgba(13,27,42,0.1)' }}>
              ...
              <div>
                ...
                <p className="text-[#0D1B2A] font-semibold text-sm">${budgetCap}</p>
              </div>
            </div>
          </div>
        ...
        <div className="flex items-center justify-between mt-8 mb-4">
          ...
          <span className="text-[#0D1B2A]/45 text-xs">Solved against ${budgetCap}</span>
        </div>

        <div ref={listRef} className="space-y-4">
          {displayOptions.map((o) => {
            const recommended = o.id === recommendedId
            const pct = Math.min(100, Math.round((o.cost / budgetCap) * 100))
            const under = budgetCap - o.cost
            return (
              <button
                key={o.id}
                onClick={() => onSelect(solved ? (o as PlanOption) : null)}
                ...
                <div className="p-5">
                  <div className="flex items-start justify-between mb-4">
                    ...
                    <div className="text-right">
                      <p className="font-display text-[#0D1B2A] text-2xl leading-none">${o.cost.toLocaleString()}</p>
                      <p className="text-xs font-semibold mt-0.5" style={{ color: under >= 0 ? '#2FB4B4' : '#FF7A00' }}>
                  ...
                  <div className="mb-4">
                    ...
                    <div className="flex items-center justify-between mt-1.5">
                      <span className="text-[#0D1B2A]/40 text-[11px]">Cost vs ${budgetCap} cap</span>
