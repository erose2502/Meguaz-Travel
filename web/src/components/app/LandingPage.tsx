...
// Card media: ambient Higgsfield loop at /videos/destinations/<slug>.mp4 when
// present; the still image doubles as poster and fallback.
const plans = [
  {
    slug: 'japan',
...
function CardMedia({ slug, img, alt }: { slug: string; img: string; alt: string }) {
  const [hasVideo, setHasVideo] = useState(true)
  return (
    <>
      {hasVideo ? (
        <video
          src={`/videos/destinations/${slug}.mp4`}
          poster={img}
          autoPlay
          muted
          loop
        ...
        <img src={img} alt={alt} className="w-full h-full object-cover transition-transform duration-[900ms] ease-out group-hover:scale-110" />
      )}
    </>
  )
}
...
export default function LandingPage({ onSearch, onViewTrips, isMobile }: LandingPageProps) {
      ...
      <div className="mx-auto w-full max-w-6xl px-5 md:px-10">
        ...
        <div className="mt-9 md:mt-14">
          <div className="io-reveal flex items-end justify-between mb-4">
            ...
            <button onClick={onViewTrips ?? onSearch} className="text-[#146C7E] text-sm font-semibold hover:text-[#0D1B2A] transition-colors">View all</button>
          </div>
            ...
            {plans.map((p, i) => (
              ...
              >
                ...
                <div className="w-28 md:w-full flex-shrink-0 rounded-2xl overflow-hidden order-2 md:order-1 md:h-44">
                  <CardMedia slug={p.slug} img={p.img} alt={p.alt} />
                </div>
              </button>
            ))}
          </div>
