...
export default function JourneyTimeline({ onBack, onBook }: JourneyTimelineProps) {
  const containerRef = useRef<HTMLDivElement>(null)
  const underBudget = BUDGET_CAP - TOTAL_COST
      ...
      <div className="sticky top-0 z-30 px-5 md:px-8 pt-12 pb-4" style={{ background: 'rgba(246,243,234,0.85)', backdropFilter: 'blur(16px)', WebkitBackdropFilter: 'blur(16px)' }}>
        <div className="mx-auto w-full max-w-3xl flex items-center gap-3">
          ...
          <div>
            ...
            <h1 className="font-display text-[#0D1B2A] text-xl leading-tight">The Balanced Route</h1>
          </div>
        </div>
      </div>
        ...
        <div className="grid md:grid-cols-2 gap-4 mt-3">
          ...
          <div className="glass-card rounded-3xl p-5">
            <div className="flex items-center gap-2 mb-3">
              ...
              <span className="ml-auto text-[#2FB4B4] text-xs font-semibold">${underBudget} to spare</span>
            </div>
            <div className="flex items-end gap-2 mb-3">
              <span className="font-display text-[#0D1B2A] text-3xl leading-none">${TOTAL_COST}</span>
              <span className="text-[#0D1B2A]/40 text-sm mb-0.5">of ${BUDGET_CAP} cap</span>
            </div>
            {/* Stacked bar */}
            <div className="h-3 rounded-full overflow-hidden flex" style={{ background: 'rgba(13,27,42,0.08)' }}>
              {costBreakdown.map((c) => (
                <div key={c.label} style={{ width: `${(c.amount / BUDGET_CAP) * 100}%`, background: c.color }} />
              ))}
            </div>
          ...
          <div className="glass-card rounded-3xl p-5">
            <div className="flex items-center gap-2 mb-3">
              ...
              <span className="ml-auto text-[#2FB4B4] text-xs font-semibold">70 min buffer</span>
            </div>
            <div className="flex items-end gap-2 mb-3">
              <span className="font-display text-[#0D1B2A] text-3xl leading-none">12h 40m</span>
            ...
            <div className="rounded-2xl px-4 py-3 flex items-center justify-between" style={{ background: 'rgba(255,122,0,0.1)' }}>
              <div>
                ...
                <p className="font-display text-[#FF7A00] text-2xl leading-none">08:15</p>
              </div>
      ...
      <div className="fixed bottom-0 inset-x-0 px-5 md:px-8 pb-8 pt-4 z-30" style={{ background: 'rgba(246,243,234,0.92)', backdropFilter: 'blur(16px)', WebkitBackdropFilter: 'blur(16px)', borderTop: '1px solid rgba(13,27,42,0.08)' }}>
        <div className="mx-auto w-full max-w-3xl flex items-center gap-4">
          <div className="hidden sm:block">
            <p className="text-[#0D1B2A]/45 text-xs">Total · ${underBudget} under budget</p>
            <p className="font-display text-[#0D1B2A] text-2xl leading-none">${TOTAL_COST}</p>
          </div>
          <button
            onClick={onBook}
